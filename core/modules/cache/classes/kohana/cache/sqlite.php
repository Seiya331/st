...
class Kohana_Cache_Sqlite extends Cache implements Cache_Tagging, Cache_GarbageCollect
{
    ...
    public function set_with_tags($id, $data, $lifetime = null, array $tags = null)
    {
        ...
        if ($lifetime === null) {
            $lifetime =
                (0 === Arr::get($this->_config, 'default_expire', null)) ? 0 : (Arr::get($this->_config,
                        'default_expire', Cache::DEFAULT_EXPIRE) + time());
        } else {
        ...
        $statement =
            $this->exists($id) ? $this->_db->prepare('UPDATE caches SET expiration = :expiration, cache = :cache, tags = :tags WHERE id = :id') : $this->_db->prepare('INSERT INTO caches (id, cache, expiration, tags) VALUES (:id, :cache, :expiration, :tags)');
